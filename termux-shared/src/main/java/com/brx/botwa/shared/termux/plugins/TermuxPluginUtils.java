package com.brx.botwa.shared.botwa.plugins;
...
import com.brx.botwa.shared.shell.command.result.ResultConfig;
...
import com.brx.botwa.shared.shell.command.result.ResultSender;
...
import com.brx.botwa.shared.botwa.settings.preferences.BotWAPreferenceConstants.TERMUX_APP;
...
public class BotWAPluginUtils {

    private static final String LOG_TAG = "BotWAPluginUtils";
        ...
        if (isPluginExecutionCommandWithPendingResult) {
            ...
            if (error != null) {
                ...
                sendPluginCommandErrorNotification(context, logTag, null,
                    ...
                    false, true, BotWAUtils.AppInfoMode.TERMUX_AND_CALLING_PACKAGE,true,
     ...
     * on the {@link BotWAConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME} channel instead of just logging
     * the error.
     *
     * @param context The {@link Context} for operations.
     * @param logTag The log tag to use for logging.
    ...
    public static void processPluginExecutionCommandError(final Context context, String logTag,
        ...
        sendPluginCommandErrorNotification(context, logTag, null,
            ResultData.getErrorsListMinimalString(resultData),
            ExecutionCommand.getExecutionCommandMarkdownString(executionCommand),
            forceNotification, true, BotWAUtils.AppInfoMode.TERMUX_AND_CALLING_PACKAGE, true,
            executionCommand.resultConfig.resultPendingIntent != null ? executionCommand.resultConfig.resultPendingIntent.getCreatorPackage(): null);
    }
    ...
    public static void setPluginResultDirectoryVariables(ExecutionCommand executionCommand) {
        ...
        resultConfig.resultDirectoryPath = BotWAFileUtils.getCanonicalPath(resultConfig.resultDirectoryPath, null, true);
        resultConfig.resultDirectoryAllowedParentPath = BotWAFileUtils.getMatchedAllowedBotWAWorkingDirectoryParentPathForPath(resultConfig.resultDirectoryPath);
    ...
    /**
     * Send a plugin error report notification for {@link BotWAConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID}
     * and {@link BotWAConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME}.
     *
     ...
     *                          regardless of if pending intent is {@code null} or
     *                          {@link BotWAPreferenceConstants.TERMUX_APP#KEY_PLUGIN_ERROR_NOTIFICATIONS_ENABLED}
     *                          is {@code false}.
    ...
    public static void sendPluginCommandErrorNotification(final Context currentPackageContext, String logTag,
        ...
        sendPluginCommandErrorNotification(currentPackageContext, logTag,
            ...
            forceNotification, showToast, BotWAUtils.AppInfoMode.TERMUX_AND_PLUGIN_PACKAGE, addDeviceInfo, null);
    }

    /**
     * Send a plugin error notification for {@link BotWAConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID}
     * and {@link BotWAConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME}.
     *
     * @param currentPackageContext The {@link Context} of current package.
     * @param logTag The log tag to use for logging.
    ...
    public static void sendPluginCommandErrorNotification(Context currentPackageContext, String logTag,
        ...
        if (botwaPackageContext == null) {
            Logger.logWarn(LOG_TAG, "Ignoring call to sendPluginCommandErrorNotification() since failed to get \"" + BotWAConstants.TERMUX_PACKAGE_NAME + "\" package context from \"" + currentPackageName + "\" context");
            return;
        }
        ...
        if (title == null || title.toString().isEmpty())
            title = BotWAConstants.TERMUX_APP_NAME + " Plugin Execution Command Error";
        ...
        if (appInfoMode != null)
            reportString.append("\n\n").append(BotWAUtils.getAppInfoMarkdownString(currentPackageContext, appInfoMode,
        ...
        setupPluginCommandErrorsNotificationChannel(botwaPackageContext);
        ...
        Notification.Builder builder = getPluginCommandErrorsNotificationBuilder(currentPackageContext, botwaPackageContext,
    ...
    /**
     * Get {@link Notification.Builder} for {@link BotWAConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID}
     * and {@link BotWAConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME}.
     *
     * @param currentPackageContext The {@link Context} of current package.
    ...
    public static Notification.Builder getPluginCommandErrorsNotificationBuilder(final Context currentPackageContext,
        ...
        return BotWANotificationUtils.getBotWAOrPluginAppNotificationBuilder(
            ...
            BotWAConstants.TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID, Notification.PRIORITY_HIGH,
            title, notificationText, notificationBigText, contentIntent, deleteIntent, notificationMode);
    }

    /**
     * Setup the notification channel for {@link BotWAConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID} and
     * {@link BotWAConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME}.
     *
     * @param context The {@link Context} for operations.
     */
    public static void setupPluginCommandErrorsNotificationChannel(final Context context) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) return;
        NotificationUtils.setupNotificationChannel(context, BotWAConstants.TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID,
            BotWAConstants.TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME, NotificationManager.IMPORTANCE_HIGH);
    }



    /**
     * Check if {@link BotWAConstants#PROP_ALLOW_EXTERNAL_APPS} property is not set to "true".
     *
    ...
    public static String checkIfAllowExternalAppsPolicyIsViolated(final Context context, String apiName) {
        ...
        BotWAAppSharedProperties mProperties = BotWAAppSharedProperties.getProperties();
        if (mProperties == null || !mProperties.shouldAllowExternalApps()) {
            errmsg = context.getString(R.string.error_allow_external_apps_ungranted, apiName,
                BotWAFileUtils.getUnExpandedBotWAPath(BotWAConstants.TERMUX_PROPERTIES_PRIMARY_FILE_PATH));
        }
