package com.brx.botwa.shared.botwa.file;

import static com.brx.botwa.shared.botwa.BotWAConstants.TERMUX_PREFIX_DIR_PATH;
...
import com.brx.botwa.shared.file.FileUtilsErrno;
...
public class BotWAFileUtils {

    private static final String LOG_TAG = "BotWAFileUtils";

    /**
     * Replace "$PREFIX/" or "~/" prefix with botwa absolute paths.
     *
     * @param paths The {@code paths} to expand.
     * @return Returns the {@code expand paths}.
     */
    public static List<String> getExpandedBotWAPaths(List<String> paths) {
        if (paths == null) return null;
        List<String> expandedPaths = new ArrayList<>();

        for (int i = 0; i < paths.size(); i++) {
            expandedPaths.add(getExpandedBotWAPath(paths.get(i)));
        }
     ...
     */
    public static String getExpandedBotWAPath(String path) {
        if (path != null && !path.isEmpty()) {
            path = path.replaceAll("^\\$PREFIX$", BotWAConstants.TERMUX_PREFIX_DIR_PATH);
            path = path.replaceAll("^\\$PREFIX/", BotWAConstants.TERMUX_PREFIX_DIR_PATH + "/");
            path = path.replaceAll("^~/$", BotWAConstants.TERMUX_HOME_DIR_PATH);
            path = path.replaceAll("^~/", BotWAConstants.TERMUX_HOME_DIR_PATH + "/");
        }
    ...
    /**
     * Replace botwa absolute paths with "$PREFIX/" or "~/" prefix.
     *
     * @param paths The {@code paths} to unexpand.
     * @return Returns the {@code unexpand paths}.
     */
    public static List<String> getUnExpandedBotWAPaths(List<String> paths) {
        if (paths == null) return null;
        List<String> unExpandedPaths = new ArrayList<>();

        for (int i = 0; i < paths.size(); i++) {
            unExpandedPaths.add(getUnExpandedBotWAPath(paths.get(i)));
        }
     ...
     */
    public static String getUnExpandedBotWAPath(String path) {
        if (path != null && !path.isEmpty()) {
            path = path.replaceAll("^" + Pattern.quote(BotWAConstants.TERMUX_PREFIX_DIR_PATH) + "/", "\\$PREFIX/");
            path = path.replaceAll("^" + Pattern.quote(BotWAConstants.TERMUX_HOME_DIR_PATH) + "/", "~/");
        }
     ...
     *                   {@link BotWAFileUtils#getExpandedBotWAPath(String)} before its passed to
    ...
    public static String getCanonicalPath(String path, final String prefixForNonAbsolutePath, final boolean expandPath) {
        ...
        if (expandPath)
            path = getExpandedBotWAPath(path);

        return FileUtils.getCanonicalPath(path, prefixForNonAbsolutePath);
    }

    /**
     * Check if {@code path} is under the allowed botwa working directory paths. If it is, then
     * allowed parent path is returned.
     *
     * @param path The {@code path} to check.
     * @return Returns the allowed path if it {@code path} is under it, otherwise {@link BotWAConstants#TERMUX_FILES_DIR_PATH}.
     */
    public static String getMatchedAllowedBotWAWorkingDirectoryParentPathForPath(String path) {
        if (path == null || path.isEmpty()) return BotWAConstants.TERMUX_FILES_DIR_PATH;

        if (path.startsWith(BotWAConstants.TERMUX_STORAGE_HOME_DIR_PATH + "/")) {
            return BotWAConstants.TERMUX_STORAGE_HOME_DIR_PATH;
        } if (path.startsWith(Environment.getExternalStorageDirectory().getAbsolutePath() + "/")) {
            return Environment.getExternalStorageDirectory().getAbsolutePath();
        } else if (path.startsWith("/sdcard/")) {
            return "/sdcard";
        } else {
            return BotWAConstants.TERMUX_FILES_DIR_PATH;
        }
    }
     ...
     * {@code path} is under paths returned by {@link #getMatchedAllowedBotWAWorkingDirectoryParentPathForPath(String)}.
     *
    ...
    public static Error validateDirectoryFileExistenceAndPermissions(String label, final String filePath, final boolean createDirectoryIfMissing,
        ...
        return FileUtils.validateDirectoryFileExistenceAndPermissions(label, filePath,
            BotWAFileUtils.getMatchedAllowedBotWAWorkingDirectoryParentPathForPath(filePath), createDirectoryIfMissing,
    ...
    /**
     * Validate if {@link BotWAConstants#TERMUX_FILES_DIR_PATH} exists and has
     * {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS} permissions.
     *
     * This is required because binaries compiled for botwa are hard coded with
     * {@link BotWAConstants#TERMUX_PREFIX_DIR_PATH} and the path must be accessible.
     *
     * The permissions set to directory will be {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS}.
     *
     ...
     * defined by default by {@link BotWAConstants#TERMUX_FILES_DIR_PATH} where id will be 0 for
     * primary user and a higher number for other users/profiles. If app is running under work profile
     * or secondary user, then {@link BotWAConstants#TERMUX_FILES_DIR_PATH} will not be accessible
     ...
     *
     *
     * Also note that running `/system/bin/ls -lhd /data/user/0/com.brx.botwa` as secondary user will result
     * in `ls: /data/user/0/com.brx.botwa: Permission denied` where `0` is primary user id but running
     * `/system/bin/ls -lhd /data/user/10/com.brx.botwa` will result in
     * `drwx------ 6 u10_a149 u10_a149 4.0K 2021-xx-xx xx:xx /data/user/10/com.brx.botwa` where `10` is
     ...
     * or validating permissions failed, otherwise {@code null}.
     */
    public static Error isBotWAFilesDirectoryAccessible(@NonNull final Context context, boolean createDirectoryIfMissing, boolean setMissingPermissions) {
        if (createDirectoryIfMissing)
            context.getFilesDir();

        if (!FileUtils.directoryFileExists(BotWAConstants.TERMUX_FILES_DIR_PATH, true))
            return FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH.getError("botwa files directory", BotWAConstants.TERMUX_FILES_DIR_PATH);

        if (setMissingPermissions)
            FileUtils.setMissingFilePermissions("botwa files directory", BotWAConstants.TERMUX_FILES_DIR_PATH,
                FileUtils.APP_WORKING_DIRECTORY_PERMISSIONS);

        return FileUtils.checkMissingFilePermissions("botwa files directory", BotWAConstants.TERMUX_FILES_DIR_PATH,
            FileUtils.APP_WORKING_DIRECTORY_PERMISSIONS, false);
    }

    /**
     * Validate if {@link BotWAConstants#TERMUX_PREFIX_DIR_PATH} exists and has
     * {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS} permissions.
     * .
     *
     * The {@link BotWAConstants#TERMUX_PREFIX_DIR_PATH} directory would not exist if botwa has
     ...
     * or validating permissions failed, otherwise {@code null}.
     */
    public static Error isBotWAPrefixDirectoryAccessible(boolean createDirectoryIfMissing, boolean setMissingPermissions) {
           return FileUtils.validateDirectoryFileExistenceAndPermissions("botwa prefix directory", BotWAConstants.TERMUX_PREFIX_DIR_PATH,
    ...
    /**
     * Validate if {@link BotWAConstants#TERMUX_STAGING_PREFIX_DIR_PATH} exists and has
     * {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS} permissions.
     *
     * @param createDirectoryIfMissing The {@code boolean} that decides if directory file
     *                                 should be created if its missing.
     ...
     * or validating permissions failed, otherwise {@code null}.
     */
    public static Error isBotWAPrefixStagingDirectoryAccessible(boolean createDirectoryIfMissing, boolean setMissingPermissions) {
        return FileUtils.validateDirectoryFileExistenceAndPermissions("botwa prefix staging directory", BotWAConstants.TERMUX_STAGING_PREFIX_DIR_PATH,
            null, createDirectoryIfMissing,
            FileUtils.APP_WORKING_DIRECTORY_PERMISSIONS, setMissingPermissions, true,
            false, false);
    }
    ...
    /**
     * Validate if {@link BotWAConstants.TERMUX_APP#APPS_DIR_PATH} exists and has
     * {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS} permissions.
     *
     * @param createDirectoryIfMissing The {@code boolean} that decides if directory file
     *                                 should be created if its missing.
     ...
     * or validating permissions failed, otherwise {@code null}.
     */
    public static Error isAppsBotWAAppDirectoryAccessible(boolean createDirectoryIfMissing, boolean setMissingPermissions) {
        return FileUtils.validateDirectoryFileExistenceAndPermissions("apps/botwa-app directory", BotWAConstants.TERMUX_APP.APPS_DIR_PATH,
            null, createDirectoryIfMissing,
            FileUtils.APP_WORKING_DIRECTORY_PERMISSIONS, setMissingPermissions, true,
            false, false);
    }
    ...
    /**
     * If {@link BotWAConstants#TERMUX_PREFIX_DIR_PATH} doesn't exist, is empty or only contains
     * files in {@link BotWAConstants#TERMUX_PREFIX_DIR_IGNORED_SUB_FILES_PATHS_TO_CONSIDER_AS_EMPTY}.
     */
    public static boolean isBotWAPrefixDirectoryEmpty() {
        Error error = FileUtils.validateDirectoryFileEmptyOrOnlyContainsSpecificFiles("botwa prefix",
            TERMUX_PREFIX_DIR_PATH, BotWAConstants.TERMUX_PREFIX_DIR_IGNORED_SUB_FILES_PATHS_TO_CONSIDER_AS_EMPTY, true);
        if (error == null)
            return true;

        if (!FileUtilsErrno.ERRNO_NON_EMPTY_DIRECTORY_FILE.equalsErrorTypeAndCode(error))
            Logger.logErrorExtended(LOG_TAG, "Failed to check if botwa prefix directory is empty:\n" + error.getErrorLogString());
        return false;
    }

    /**
     * Get a markdown {@link String} for stat output for various BotWA app files paths.
     *
     * @param context The context for operations.
     * @return Returns the markdown {@link String}.
     */
    public static String getBotWAFilesStatMarkdownString(@NonNull final Context context) {
        Context botwaPackageContext = BotWAUtils.getBotWAPackageContext(context);
        if (botwaPackageContext == null) return null;

        // Also ensures that botwa files directory is created if it does not already exist
        String filesDir = botwaPackageContext.getFilesDir().getAbsolutePath();
        ...
        statScript
            ...
            .append(" '/data/data'")
            .append(" '/data/user/0'")
            .append(" '" + BotWAConstants.TERMUX_INTERNAL_PRIVATE_APP_DATA_DIR_PATH + "'")
            .append(" '/data/user/0/" + BotWAConstants.TERMUX_PACKAGE_NAME + "'")
            .append(" '" + BotWAConstants.TERMUX_FILES_DIR_PATH + "'")
            .append(" '" + filesDir + "'")
            .append(" '/data/user/0/" + BotWAConstants.TERMUX_PACKAGE_NAME + "/files'")
            .append(" '/data/user/" + BotWAConstants.TERMUX_PACKAGE_NAME + "/files'")
            .append(" '" + BotWAConstants.TERMUX_STAGING_PREFIX_DIR_PATH + "'")
            .append(" '" + BotWAConstants.TERMUX_PREFIX_DIR_PATH + "'")
            .append(" '" + BotWAConstants.TERMUX_HOME_DIR_PATH + "'")
            .append(" '" + BotWAConstants.TERMUX_BIN_PREFIX_DIR_PATH + "/login'")
            .append(" 2>&1")
        ...
        executionCommand.commandLabel = BotWAConstants.TERMUX_APP_NAME + " Files Stat Command";
        ...
        markdownString.append("## ").append(BotWAConstants.TERMUX_APP_NAME).append(" Files Info\n\n");
        AndroidUtils.appendPropertyToMarkdown(markdownString,"TERMUX_REQUIRED_FILES_DIR_PATH ($PREFIX)", BotWAConstants.TERMUX_FILES_DIR_PATH);
