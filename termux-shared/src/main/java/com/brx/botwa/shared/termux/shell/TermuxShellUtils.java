...
import com.brx.botwa.shared.file.filesystem.FileTypes;
...
public class BotWAShellUtils {

    private static final String LOG_TAG = "BotWAShellUtils";
            ...
            try (FileInputStream in = new FileInputStream(file)) {
                ...
                if (bytesRead > 4) {
                    if (buffer[0] == 0x7F && buffer[1] == 'E' && buffer[2] == 'L' && buffer[3] == 'F') {
                            ...
                            if (c == ' ' || c == '\n') {
                                if (builder.length() == 0) {
                                    ...
                                    if (shebangExecutable.startsWith("/usr") || shebangExecutable.startsWith("/bin")) {
                                        ...
                                        interpreter = BotWAConstants.TERMUX_BIN_PREFIX_DIR_PATH + "/" + binary;
                                    }
                                    break;
                                }
                            } else {
                        ...
                        interpreter = BotWAConstants.TERMUX_BIN_PREFIX_DIR_PATH + "/sh";
                    }
                }
            }
        } catch (IOException e) {
    ...
    /** Clear files under {@link BotWAConstants#TERMUX_TMP_PREFIX_DIR_PATH}. */
    public static void clearBotWATMPDIR(boolean onlyIfExists) {
        ...
        // botwa-reset (d6eb5e35). Moreover, TMPDIR must be a directory and not a symlink, this can
        // also allow users who don't want TMPDIR to be cleared automatically on botwa exit, since
        // it may remove files still being used by background processes (#1159).
        if(onlyIfExists && !FileUtils.directoryFileExists(BotWAConstants.TERMUX_TMP_PREFIX_DIR_PATH, false))
            return;
        ...
        if (days < 0) {
            Logger.logInfo(LOG_TAG, "Not clearing botwa $TMPDIR");
        } else if (days == 0) {
            error = FileUtils.clearDirectory("$TMPDIR",
                FileUtils.getCanonicalPath(BotWAConstants.TERMUX_TMP_PREFIX_DIR_PATH, null));
            if (error != null) {
                Logger.logErrorExtended(LOG_TAG, "Failed to clear botwa $TMPDIR\n" + error);
            }
        } else {
            ...
            if (error != null) {
                Logger.logErrorExtended(LOG_TAG, "Failed to delete files from botwa $TMPDIR older than " + days + " days\n" + error);
            }
        }
    }
