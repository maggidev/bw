...
public class BotWABootstrap {

    private static final String LOG_TAG = "BotWABootstrap";

    /** The field name used by BotWA app to store package variant in
    ...
    public static void setBotWAPackageManagerAndVariant(@Nullable String packageVariantName) {
     ...
     * {@link TERMUX_APP#BUILD_CONFIG_CLASS_NAME} class of the BotWA app APK installed on the device.
     * This can only be used by apps that share `sharedUserId` with the BotWA app and can be used
     * by plugin apps.
     *
     * @param currentPackageContext The context of current package.
     */
    public static void setBotWAPackageManagerAndVariantFromBotWAApp(@NonNull Context currentPackageContext) {
        String packageVariantName = getBotWAAppBuildConfigPackageVariantFromBotWAApp(currentPackageContext);
        if (packageVariantName != null) {
            BotWABootstrap.setBotWAPackageManagerAndVariant(packageVariantName);
        } else {
            Logger.logError(LOG_TAG, "Failed to set TERMUX_APP_PACKAGE_VARIANT and TERMUX_APP_PACKAGE_MANAGER from the botwa app");
        }
    }
     ...
     */
    public static String getBotWAAppBuildConfigPackageVariantFromBotWAApp(@NonNull Context currentPackageContext) {
        try {
            return (String) BotWAUtils.getBotWAAppAPKBuildConfigClassField(currentPackageContext, BUILD_CONFIG_FIELD_TERMUX_PACKAGE_VARIANT);
        } catch (Exception e) {
    ...
    public enum PackageManager {
        ...
        ///**
        // * BotWA Android Package Manager (TAPM) for managing botwa apk package files.
    ...
    /** BotWA package variant. The substring before first dash "-" must match one of the {@link PackageManager}. */
