package com.brx.botwa.app.event;
...
public class SystemEventReceiver extends BroadcastReceiver {
    ...
    public synchronized void onActionBootCompleted(@NonNull Context context, @NonNull Intent intent) {
        BotWAShellManager.onActionBootCompleted(context, intent);
    }

    public synchronized void onActionPackageUpdated(@NonNull Context context, @NonNull Intent intent) {
        Uri data = intent.getData();
        if (data != null && BotWAUtils.isUriDataForBotWAPluginPackage(data)) {
            ...
            if (BotWAFileUtils.isBotWAFilesDirectoryAccessible(context, false, false) == null)
                BotWAShellEnvironment.writeEnvironmentToFile(context);
        }
    }
