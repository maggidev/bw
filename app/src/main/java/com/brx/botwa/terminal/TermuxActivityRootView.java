...
/**
 * The {@link BotWAActivity} relies on {@link android.view.WindowManager.LayoutParams#SOFT_INPUT_ADJUST_RESIZE)}
 * set by {@link BotWATerminalViewClient#setSoftKeyboardState(boolean, boolean)} to automatically
 * resize the view and push the terminal up when soft keyboard is opened. However, this does not
 * always work properly. When `enforce-char-based-input=true` is set in `botwa.properties`
 * and {@link com.brx.botwa.view.TerminalView#onCreateInputConnection(EditorInfo)} sets the inputType
 * to `InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD | InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS`
 * instead of the default `InputType.TYPE_NULL` for botwa, some keyboards may still show suggestions.
 ...
 * Also check https://github.com.brx.botwa/botwa-app/issues/1539.
 *
 * This overlap may happen even without `enforce-char-based-input=true` for keyboards with extended layouts
 * like number row, etc.
 *
 * To fix these issues, `activity_botwa.xml` has the constant 1sp transparent
 * `activity_botwa_bottom_space_view` View at the bottom. This will appear as a line matching the
 * activity theme. When {@link BotWAActivity} {@link ViewTreeObserver.OnGlobalLayoutListener} is
 ...
 */
public class BotWAActivityRootView extends LinearLayout implements ViewTreeObserver.OnGlobalLayoutListener {

    public BotWAActivity mActivity;
    ...
    private static final String LOG_TAG = "BotWAActivityRootView";

    private static int mStatusBarHeight;

    public BotWAActivityRootView(Context context) {
        super(context);
    }

    public BotWAActivityRootView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public BotWAActivityRootView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public void setActivity(BotWAActivity activity) {
        mActivity = activity;
    }
    ...
    public void onGlobalLayout() {
        ...
        View bottomSpaceView = mActivity.getBotWAActivityBottomSpaceView();
        ...
        if (isVisible) {
            ...
            // Calling addBotWAActivityRootViewGlobalLayoutListener with a delay fails to
